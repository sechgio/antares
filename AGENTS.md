...
- El workflow `review-policy` audita cada PR en push, edición de descripción y cambio de etiquetas (`scripts/review-policy-check.js`, ejecutable en local con `--pr <n>`): exige intención real en el cuerpo (la plantilla sin rellenar no cuenta), tamaño efectivo < 1000 líneas excluyendo lockfiles y generados (la etiqueta `size/exempt` es la excepción) y ningún archivo nuevo > 500 líneas fuera de tests/generados. En borrador todo degrada a aviso; el informe vive en un único comentario auto-actualizado y la taxonomía esperada del revisor es `blocking:`/`suggestion:`/`nit:`/`question:`/`praise:`.

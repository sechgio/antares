/**
 * Store consolidado de AutoIMG: invariantes del refactor.
 * - Paridad de comportamiento con los wrappers históricos
 * - Semántica de logout: clearTokens() borra SOLO tokens; sheet y prefs sobreviven
 * - Guards sin usuario activo (no exponer ni escribir datos de perfil)
 * - OAuth config global (independiente del usuario)
 */
...
// Merge parcial en saveLocalPrefs
...
// ---------------------------------------------------------------------------
// Tokens: sanitización a 3 campos
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// clearTokensLegacyPaths: borra restos anonymous + legacy root
// ---------------------------------------------------------------------------

scope.clearActiveUser();
store.saveTokens({ access_token: 'anon', refresh_token: 'r', expiry_date: 1 }); // → autoimg/anonymous/tokens.json
...
// ---------------------------------------------------------------------------
// Sin usuario activo: guards de sheet y prefs
// ---------------------------------------------------------------------------

assert(!store.loadSheetConfig().sheet_id, 'sin usuario no expone sheet');
assert(store.loadLocalFolders().length === 0, 'sin usuario no expone carpetas');

// Guard de ESCRITURA sin usuario: saveSheetConfig/saveLocalPrefs son no-op
...
// clearSheetConfig/clearLocalPrefs no afectan tokens
...
// RevokeAuth no debe llamar clearSheetConfig (regresión de logout)

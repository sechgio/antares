...
def run_conversion_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    ...
    src_dir = workdir / "conv-src"
    src_dir.mkdir(parents=True, exist_ok=True)

    def transform(item: dict[str, Any]) -> dict[str, Any]:
        job, image = item["job"], item["image"]
        src = src_dir / image["filename"]
        if not src.exists():
            Image.new("RGB", (image["width"], image["height"]), (30, 90, 160)).save(src, format="PNG")
        convertir_imagen(src, workdir / "conv-out" / f"{job['id']}.webp", "webp", calidad=job["options"]["quality"])
        return {
            "job_id": job["id"],
            "input": image["filename"],
            "output_mime": job["options"]["target_mime"],
            "estimated_output_bytes": image["payload"]["byte_length"] * 3 // 5,
        }
...
def run_list_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core import history as history_core

    for record in fixtures["sqlite_records"]:
        history_core.save_run(
            [record["columns"]["source_name"]],
            {"formato": record["columns"]["target_format"], "calidad": 90, "resize": None, "keep_exif": False},
            "baseline",
            record["columns"]["target_format"],
            90,
            None,
            1,
            0,
            duration_ms=record["columns"]["output_bytes"] % 500,
        )

    def transform(record: dict[str, Any]) -> dict[str, Any]:
        history_core.list_runs(limit=50)
        return {
...
def run_export_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core.json_store import JsonDocumentStore

    store = JsonDocumentStore(workdir / "exports.json", lambda doc: dict(doc))
...
def run_spreadsheet_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core.mapping_index import MappingIndex

    index = MappingIndex({f"{row['cells']['account']}.xlsx": row["cells"]["account"] for row in fixtures["spreadsheet_rows"]})

    def transform(row: dict[str, Any]) -> dict[str, Any]:
        index.lookup(f"{row['cells']['account']}.xlsx")
        return {
...
def run_canvas_sync_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core.canvas.store import CanvasStore

    store = CanvasStore(workdir / "canvas-documents", migrate_legacy=False)
...
def run_espacios_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core.json_store import JsonDocumentStore

    users = {user["id"]: user for user in fixtures["users"]}
    store = JsonDocumentStore(workdir / "espacios-tasks.json", lambda doc: dict(doc))
...
def _measure_real_queue_wait(scheduler: Any, clock: Clock) -> float:
    blocker = scheduler.submit_light(lambda: time.sleep(0.05))
    started = clock()
    probe = scheduler.submit_light(lambda: None)
    probe.result(timeout=30)
    queue_wait_ms = max(0.0, (clock() - started) * 1_000)
    blocker.result(timeout=30)
    return queue_wait_ms


def run_autoimg_scenario(fixtures: dict[str, Any], scale: str, *, clock: Clock, rss_sampler: RssSampler, workdir: Path) -> dict[str, Any]:
    from backend.core.scheduler import WorkScheduler

    scheduler = WorkScheduler(light_workers=1, heavy_workers=1, heavy_queue_limit=8)
    try:
        images = {image["id"]: image for image in fixtures["images"]}
        queue_wait_ms = _measure_real_queue_wait(scheduler, clock)
        items = [{"job": job, "image": images[job["source_image_id"]]} for job in fixtures["concurrent_jobs"]]
        ...
        def transform(item: dict[str, Any]) -> dict[str, Any]:
            ...
            payload = {
                ...
                "source_checksum": image["payload"]["checksum"],
                "target_mime": job["options"]["target_mime"],
                "quality": job["options"]["quality"],
            }
            return scheduler.submit_light(lambda: payload).result(timeout=30)
...
def run_offline_baseline(
    ...
    *,
    ...
    work = Path(workdir) if workdir is not None else Path(tempfile.mkdtemp(prefix="antares-scalability-"))
    work.mkdir(parents=True, exist_ok=True)
    previous_localappdata = os.environ.get("LOCALAPPDATA")
    os.environ["LOCALAPPDATA"] = str(work)
    try:
        measurements = [
            runner(fixtures["data"], scale, clock=clock, rss_sampler=sampler, workdir=work)
            for runner in SCENARIO_RUNNERS.values()
        ]
    finally:
...
def serialize_result(*, fixtures: dict[str, Any], measurements: list[dict[str, Any]], metadata: dict[str, Any]) -> dict[str, Any]:
    result = {"schema_version": 1, "mode": "offline-real-paths", "fixtures": fixtures, "measurements": measurements, "metadata": metadata}

...
describe("preview panel PDF export helpers", () => {
  ...
  it("selects all rows with matching images for consolidated export", () => {
    ...
    expect(selected[0].images.map((img) => img.name)).toEqual([
      "A1_1.jpg",
      "A1_2.jpg",
    ]);
  });
  ...
  it("single export rechaza índices inválidos y usa rowIndex sin idColumn", () => {
    const rows = [{ OT: "A1" }];
    expect(
      selectRowsForPdfExport({
        data: rows,
    ...
    const noId = selectRowsForPdfExport({
      data: rows,
      selectedIndex: "0",
      exportScope: "single",
      idColumn: "",
  ...
  it("requiresImages sin idColumn o sin idValue filtra filas sin imágenes", () => {
    ...
    expect(
      selectRowsForPdfExport({
        ...
        idColumn: "",
        requiresImages: true,
        images: [file("A1_1.jpg")],
      }),
    ).toEqual([]);
    const emptyId = selectRowsForPdfExport({
      data: rows,
      selectedIndex: "",
      exportScope: "all",
      idColumn: "OT",
    ...
    expect(emptyId.map((i) => i.idValue)).toEqual(["A1"]);
  });

  it("filename sin templateName usa panel_fotografico", () => {
    expect(
      buildPdfFilename({
        exportScope: "single",
        templateName: null,
    ...
    ).toBe("panel_fotografico_X.pdf");
  });

  it("merge usa fallback regex cuando DOMParser no existe", () => {
    vi.stubGlobal("DOMParser", undefined);
    const html = mergeHtmlDocuments([
      "<html><head><style>.x{}</style></head><body><p>A</p></body></html>",
    ]);

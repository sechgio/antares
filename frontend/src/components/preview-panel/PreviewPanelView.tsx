...
import { validateTemplateStructure } from "./utils";
import {
  computeAutoMappings,
...
export default function PreviewPanelView() {
  ...
  const loadTemplates = useCallback(async () => {
    try {
      ...
      const templates = (res.templates || []).filter(
        (t) =>
          t.source !== "canvas" &&
          !String(t.filename || "").startsWith("canvas:"),
      );
  ...
  const handleTemplateUpload = async (
    ...
    try {
      ...
      if (validation.valid) {
        ...
        setRequiresImages(
          lc.includes("report.images") ||
            lc.includes("photo-grid") ||
            lc.includes("panel-fotografico"),
        );
      } else {
  ...
  const autoMapFields = useCallback((_headers: string[]) => {
    const { mappings, idColumn: fallbackId } = computeAutoMappings(_headers);
  ...
  const loadSheetData = useCallback(
    (sheet: { name: string; rows: unknown[][] }) => {
      const result = rowsToRecords(sheet.rows);
      if ("error" in result) {
  ...
  const fetchSheetRows = useCallback(
    async (token: string, sheetName: string): Promise<unknown[][]> => {
      const rows: unknown[][] = [];
      let offset = 0;
      for (;;) {
    ...
    async (nextName: string) => {
      ...
      if (sh && sh.rows.length === 0 && spillToken) {
        try {
          ...
          addToast({
            message: msg || "No se pudo cargar la hoja",
            type: "error",
          });
          return;
        }
  ...
  const applyParsedSheets = useCallback(
    (
      ...
      if (!allSheets.length) {
        addToast({
          message: "El archivo está vacío o no tiene hojas",
          type: "error",
        });
        return;
      }
  ...
  const parseFile = async (file: File) => {
    ...
    const win = window as unknown as {
      electronAPI?: {
        ...
        fileStagedAppend: (
          t: string,
          c: ArrayBuffer | Uint8Array | string,
        ) => Promise<unknown>;
    ...
    if (!win.electronAPI?.fileStagedCreate) {
      addToast({
        message: "Importar Excel requiere la app de escritorio Antares",
        type: "error",
      });
      return;
    }
    ...
    try {
      const { stageAndParseSpreadsheet } =
        await import("../../utils/spreadsheet");
      ...
      const msg = errorMessage(err, String(err));
      addToast({
        message: msg || "No se pudo leer el archivo Excel/CSV",
        type: "error",
      });
    }
  };
  ...
  const addCustomColumn = () => {
    ...
    const error = validateNewCustomColumn(
      newColumnName,
      newColumnMapping,
      allNames,
    );
  ...
  const handleCustomColumnKeyDown = (
  ...
  const filteredImages = useMemo(
    () => filterImagesForRecord(images, data, selectedIndex, idColumn),
  ...
  const handleDownloadPdf = async () => {
    if (exportScope === "single" && selectedIndex === "") {
      addToast({
        message: "Selecciona una fila antes de descargar el PDF.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      const documents = await mapWithConcurrencyLimit(
        selectedRows,
        2,
        async (item) => {
          const imageSources = await mapWithConcurrencyLimit(
            item.images,
            4,
            (img, imageIndex) =>
              imageToPdfSource(
                img,
                pdfQuality,
                `row-${item.rowIndex}-img-${imageIndex}`,
              ),
          );
      ...
      const res = await api.htmlToPdf({
        ...
        outputPath,
        localImagePaths:
          Object.keys(localImagePaths).length > 0 ? localImagePaths : undefined,
      });
      addToast({
      ...
      const message = errorMessage(err, "No se pudo generar el PDF.");
  ...
  return (
    <div className="flex h-full w-full bg-[var(--bg-base)] text-[var(--text-primary)] overflow-hidden">
      <aside
        className={`flex flex-col bg-[var(--bg-surface)] border-r border-[var(--border-subtle)] transition-all duration-300 ${isFocusMode ? "w-0 overflow-hidden opacity-0 border-none" : "w-[340px]"}`}
      >
          ...
          <div className="flex items-center gap-2 w-full">
            <div className="flex-1 flex gap-1" aria-hidden>
              {stepStates.map((done, i) => (
                <span
                  key={i}
                  className={`h-1 flex-1 rounded-full transition-colors duration-300 ${done ? "bg-[var(--accent-green)]" : "bg-[var(--border-medium)]"}`}
                />
              ))}
            </div>
            <span className="text-[9px] font-semibold text-[var(--text-muted)] tabular-nums shrink-0 leading-none">
              {completedCount}/6
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
          <Step
            number="0"
            title="Logos y Cabecera"
            icon={<Settings size={12} />}
            defaultOpen={!!(logoLeft || logoRight)}
            status={stepStates[0] ? "done" : "pending"}
          >
            <div className="grid grid-cols-2 gap-1.5">
              {(["left", "right"] as const).map((side) => {
                const logo = side === "left" ? logoLeft : logoRight;
                const inputId =
                  side === "left" ? "logoLeftInput" : "logoRightInput";
                return (
                  <div
                    key={side}
          ...
          <Step
            ...
            badge={
              templateStatus === "valid" ? (
                <CheckCircle size={11} className="text-[var(--accent-green)]" />
              ) : templateStatus === "invalid" ? (
                <AlertCircle size={11} className="text-[var(--accent-red)]" />
              ) : null
            }
          ...
          >
            <div className="space-y-1.5">
              ...
              <TemplatePicker
                ...
                value={
                  availableTemplates.some(
                    (t) => t.filename === customTemplate?.name,
                  )
                    ? (customTemplate?.name ?? "")
                    : ""
                }
              ...
              {templateStatus === "invalid" && templateError && (
                <div className="text-[9px] text-[var(--accent-red)] px-0.5">
                  ⚠️ {templateError}
                </div>
              )}
                ...
                {customTemplate && (
                  ...
                  >
                    <Button
                      variant="none"
                      size="none"
                      aria-label="Usar plantilla predeterminada"
                      onClick={handleResetTemplate}
                      className="shrink-0 p-1 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-elevated)] transition-colors"
                    >
                      <RotateCcw size={12} />
                    </Button>
                  </WithHoverTooltip>
              ...
              <div className="flex items-center justify-between px-2 py-1 rounded-md border border-[var(--border-medium)] bg-[var(--bg-elevated)]">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    aria-label="La plantilla requiere imágenes"
          ...
          <Step
            ...
            badge={
              data.length > 0 ? (
                <span className="text-[9px] text-[var(--accent-green)] font-medium">
                  {data.length}
                </span>
              ) : null
            }
          ...
          >
            <div className="space-y-1.5">
              <label className="block w-full cursor-pointer">
                ...
                >
                  <span
                    className={`text-[10px] ${dragStep2 ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}
                  >
                    {dragStep2
                      ? "Soltar aquí"
                      : headers.length > 0
              ...
              {sheets.length > 1 && (
                <div>
                  <label className="block text-[var(--text-muted)] text-[9px] mb-0.5 font-semibold uppercase">
                    Hoja
                  </label>
                  <TemplatePicker
                    aria-label="Hoja"
                    ...
                    options={sheets.map((s) => {
                      ...
                      return {
                        value: s.name,
                        label: `${s.name} (${Math.max(0, n - 1)} filas)`,
                      };
                    })}
                    onChange={(v) => {
                      void handleSheetChange(v);
                    }}
                  />
                </div>
              )}
              {data.length > 0 && (
                <Button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 border border-[var(--border-medium)] hover:border-[var(--text-secondary)] rounded-md py-1 text-center hover:bg-[var(--bg-elevated)] transition-all text-[10px] text-[var(--text-secondary)]"
                >
          ...
          >
            <div className="space-y-1.5">
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto pr-0.5">
                {REPORT_FIELDS.map((field) => {
                  ...
                  return (
                    <div
                      key={field.id}
                      className="grid grid-cols-[80px_1fr] gap-1.5 items-center"
                    >
                      <span
                        className="text-[var(--text-muted)] text-[9px] uppercase font-medium truncate"
                        title={field.label}
                      >
                        {field.label}
                      </span>
                      <TemplatePicker
                        ...
                        options={columnOptions}
                        onChange={(next) =>
                          setMappings((prev) => ({ ...prev, [field.id]: next }))
                        }
                        triggerClassName={
                          mapped
                ...
                {customColumns.map((col) => {
                  ...
                  return (
                    <div
                      key={col.id}
                      className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_auto] gap-1.5 items-center bg-[var(--bg-elevated)] rounded px-1.5 py-0.5"
                    >
                      <span
                        className="text-[var(--text-primary)] text-[9px] uppercase font-medium truncate"
                        title={col.name}
                      >
                        {col.name}
                      </span>
                      <TemplatePicker
                        ...
                        options={columnOptions}
                        onChange={(next) =>
                          setMappings((prev) => ({ ...prev, [col.id]: next }))
                        }
                        triggerClassName={
                          mapped
              ...
              <Button
                ...
                className="w-full border border-dashed border-[var(--border-medium)] hover:border-[var(--text-secondary)] text-[var(--text-muted)] hover:text-[var(--text-primary)] rounded-md py-1 text-center hover:bg-[var(--bg-elevated)] transition-all flex items-center justify-center gap-1.5 text-[10px]"
              >
          ...
          >
            {requiresImages ? (
              <label className="block w-full cursor-pointer">
                ...
                >
                  <span
                    className={`text-[10px] ${dragStep4 ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}
                  >
                    {dragStep4
                      ? "Soltar aquí"
                      : images.length > 0
                        ? `${images.length} imágenes`
                        : "Subir Carpeta de Fotos"}
                  </span>
                </div>
                <input
                  type="file"
              ...
              <div className="space-y-1.5 border-t border-[var(--border-subtle)] pt-1.5">
                ...
                <div className="space-y-1.5 pt-0.5">
                  <Button
                    ...
                    disabled={
                      (exportScope === "single" && selectedIndex === "") ||
                      data.length === 0 ||
                      isPdfLoading
                    }
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-[var(--text-secondary)] text-[11px] mb-1 font-medium uppercase">
                  ...
                  <input
                    ...
                    className="w-full h-8 rounded-lg border border-[var(--border-medium)] bg-[var(--bg-elevated)] px-3 text-[12px] text-[var(--text-primary)] outline-none focus:border-[var(--accent-primary)]"
                  />
                </div>
                <div>
                  <label className="block text-[var(--text-secondary)] text-[11px] mb-1 font-medium uppercase">
                    Columna del Excel a Mapear
                  </label>
                  <TemplatePicker
              ...
              <div className="flex gap-3 mt-4">
                <Button
                  ...
                  className="flex-1 border border-[var(--border-medium)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--text-secondary)] rounded-lg py-2 text-[12px] transition-colors"
                >
                  Cancelar
                </Button>
                <Button
                  ...
                  size="none"
                  type="submit"
                  className="flex-1 bg-[var(--accent-primary)] hover:bg-[var(--accent-primary-hover)] text-[var(--text-on-accent)] rounded-lg py-2 text-[12px] font-semibold transition-colors"
                >
                  Agregar
                </Button>
              </div>
          ...
          <>
            <Button
              ...
              aria-label={
                canPrevRow
                  ? "Registro anterior"
                  : "Registro anterior (deshabilitado)"
              }
              onClick={goToPrevRow}
              disabled={!canPrevRow}
              className={`fixed left-4 top-1/2 -translate-y-1/2 p-2 transition-colors z-[100] ${!canPrevRow ? "text-[var(--text-muted)] opacity-30 cursor-not-allowed" : "text-[var(--accent-primary)] hover:opacity-100 opacity-70"}`}
            >
            ...
            <Button
              ...
              aria-label={
                canNextRow
                  ? "Registro siguiente"
                  : "Registro siguiente (deshabilitado)"
              }
              onClick={goToNextRow}
              disabled={!canNextRow}
              className={`fixed right-4 top-1/2 -translate-y-1/2 p-2 transition-colors z-[100] ${!canNextRow ? "text-[var(--text-muted)] opacity-30 cursor-not-allowed" : "text-[var(--accent-primary)] hover:opacity-100 opacity-70"}`}
            >
              <ChevronRight size={64} strokeWidth={1.5} />
            </Button>
            <div
              className="fixed top-4 right-4 z-[100] text-[var(--text-muted)] text-[10px] font-mono pointer-events-none select-none px-3 py-1 rounded-full"
              style={{
                backgroundColor:
            ...
            <div
              className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] text-[var(--text-muted)] text-sm font-mono px-4 py-2 rounded-full"
              style={{
                backgroundColor:
                  "color-mix(in srgb, var(--bg-base) 40%, transparent)",
              }}
            >
              {selectedIndex !== ""
                ? `${parseInt(selectedIndex) + 1} / ${data.length}`
